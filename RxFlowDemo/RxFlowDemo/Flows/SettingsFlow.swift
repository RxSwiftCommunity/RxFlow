...
class SettingsFlow: Flow {
    ...
    func navigate(to step: Step) -> [NavigationItem] {
        ...
        case .settings:
            ...
            return [NavigationItem(nextPresentable: navigationController, nextStepper: settingsListViewController),
                    NavigationItem(nextPresentable: settingsViewController, nextStepper: settingsViewController)]
        case .apiKey:
            let settingsViewController = SettingsViewController.instantiate()
            settingsViewController.title = "Api Key"
            self.rootViewController.showDetailViewController(settingsViewController, sender: nil)
